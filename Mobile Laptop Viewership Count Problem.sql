CREATE TABLE viewership (device_type VARCHAR(255), viewership_count INTEGER);
...
Output the total viewership for laptops as laptop_views 
...
SELECT 
    SUM(CASE 
            WHEN device_type IN ('tablet', 'phone') THEN viewership_count 
            ELSE 0 
        END) AS mobile_views,
    SUM(CASE 
            WHEN device_type = 'laptop' THEN viewership_count 
            ELSE 0 
        END) AS laptop_views
FROM viewership;
